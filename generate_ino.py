...
if __name__ == "__main__":
    ...
    parser.add_argument("name", help="name of the created ino program (folder + file)")
    ...
    with open(f"{args.name}/{args.name}.ino", "w") as file:
        print("Generating ino file...")
        file.write(generate(SENSORS, CONTROLS))
        print("done")

    with open(f"{args.name}/configuration.h", "w") as file:
        print("Generating configuration header file...")
        file.write(f"""
#define PIN1 12
""")
        print("done")
